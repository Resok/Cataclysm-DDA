    ...
    for( const vpart_reference &vp : get_all_parts() ) {
        ...
        if( pt.is_reactor() ) {
            ...
            if( veh_fuel_mult == 100 ) { // Mint condition vehicle
                pt.ammo_set( "plut_cell" );
            } else if( one_in( 2 ) && veh_fuel_mult > 0 ) { // Randomize charge a bit
                pt.ammo_set( "plut_cell", pt.ammo_capacity( plut ) * ( veh_fuel_mult + rng( 0, 10 ) ) / 100 );
            } else if( one_in( 2 ) && veh_fuel_mult > 0 ) {
                pt.ammo_set( "plut_cell", pt.ammo_capacity( plut ) * ( veh_fuel_mult - rng( 0, 10 ) ) / 100 );
            } else {
                pt.ammo_set( "plut_cell", pt.ammo_capacity( plut ) * veh_fuel_mult / 100 );
            }
        }
        ...
        if( pt.is_battery() ) {
            ...
            if( veh_fuel_mult == 100 ) { // Mint condition vehicle
                ...
                pt.ammo_set( "battery", pt.ammo_capacity( battery ) * ( veh_fuel_mult + rng( 0, 10 ) ) / 100 );
            } else if( one_in( 2 ) && veh_fuel_mult > 0 ) {
                pt.ammo_set( "battery", pt.ammo_capacity( battery ) * ( veh_fuel_mult - rng( 0, 10 ) ) / 100 );
            } else {
                pt.ammo_set( "battery", pt.ammo_capacity( battery ) * veh_fuel_mult / 100 );
            }
        }

        if( type->parts[p].fuel != "null" ) {
